...
from uniswap_oracle_new import UniswapOracleNew
import random
import numpy as np
...
class TestUniswapOracle:
    ...
    def test_compare(self):
        prices = []
        prices_new = []
        o = UniswapOracle(window_size=16*3600, max_window_size=24*3600, granularity=4)
        o_new = UniswapOracleNew(window_size=16*3600, max_window_size=24*3600, granularity=4)
        ...
        for state in states:
            o.update_result(state)
            prices.append(o.median_price)

            o_new.update_result(state)
            prices_new.append(o_new.median_price)
        print(f"took {time.time() - s} secs")
       
        print(prices)
        print(prices_new)
        #assert np.isclose(prices, prices_new).all()
        assert prices == prices_new

@pytest.mark.skip('tmp')
class TestUniswapOracleNew:
    def test_speed(self):
        o = UniswapOracleNew()
