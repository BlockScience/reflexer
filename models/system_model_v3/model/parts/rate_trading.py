...
def p_trade_rate(params, substep, state_history, state):
    ...
    APY = 0
    #print(f"{state['timestep']}, {redemption_rate=}")
    try:
        APY = ((1 + Decimal(redemption_rate)) ** (60*60*24*356) - 1) * 100
    except FloatingPointError as e:
        print(f"FloatingPointError at {state['timestep']}")
        raise e
    #print(f"{state['timestep']}, {APY=}")
    very_negative_rate = redemption_rate < 0 and APY < -params['rate_trader_apy_bound']
    very_positive_rate = redemption_rate > 0 and APY > params['rate_trader_apy_bound']
