...
def update_target_rated(params, substep, state_history, state, policy_input):
    ...
    if state['cumulative_time'] % params['control_period'] == 0:
        ...
        prev_error = state["prev_error_star"]  # unit USD
        error_integral = state["error_star_integral"]  # unit USD * seconds

        #print(f"adding {params['kp'] * error } to rate")
        target_rate = state['target_rate'] + params["kp"] * error + params["ki"] / params['control_period'] * error_integral + params["kd"] * (error - prev_error)/params['control_period']
...
def update_target_rate(params, substep, state_history, state, policy_input):
    ...
    if state['cumulative_time'] % params['control_period'] == 0:
        ...
        #print(f"adding {params['kp'] * error } to rate")
        target_rate = params["kp"] * error + params["ki"] / params['control_period'] * error_integral + params["kd"] * (error - prev_error)/params['control_period']
...
def observe_errors(params, substep, state_history, state):
    ...
    #print(f"cum {state['cumulative_time']}")
