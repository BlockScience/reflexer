...
'''
system_simulation = ConfigWrapper(system_model, M=update_params, N=MONTE_CARLO_RUNS, T=SIMULATION_TIMESTEPS)
...
fig = make_subplots(
    ...
    column_titles=['Shock', 'Response', 'Market price / Debt price', 'Over/undershoot', 'Recovery metric'],
    row_titles=[
        'Impulse 0.01', 'Step 0.01', 'Sinusoid 0.01',
        'Impulse 0.5', 'Step 0.5', 'Sinusoid 0.5',
        'Impulse 1.0', 'Step 1.0', 'Sinusoid 1.0',
        '3600s control delay',
        '7200s control delay',
...
for subset in range(df.subset.max() + 1):
    ...
    col = 1
    fig.add_trace(go.Scatter(name='debt_price', x=dataset[dataset.run == 1].timestep, y=dataset[dataset.run == 1]['debt_price'], line_color='black', showlegend=False),
                  row=row, col=col)
    
    col = 2
    fig.add_trace(go.Scatter(name='debt_price', x=dataset['timestep'], y=mean['debt_price'], line_color='black', showlegend=show_legend),
                  row=row, col=col)
    fig.add_trace(go.Scatter(name='target_price', x=dataset['timestep'], y=mean['target_price'], line_color='red', showlegend=show_legend),
                  row=row, col=col)
    fig.add_trace(go.Scatter(name='market_price', x=dataset['timestep'], y=mean['market_price'], line_color='green', showlegend=show_legend),
                  row=row, col=col)
    
    col = 3    
    fig.add_trace(go.Scatter(x=timesteps, y=mean['relative_diff'] + std['relative_diff'], name='mean + std', mode='lines', line_color='blue', showlegend=show_legend),
                  row=row, col=col)
    fig.add_trace(go.Scatter(x=timesteps, y=mean['relative_diff'], name='mean', fill='tonexty', mode='lines', line_color='blue', showlegend=show_legend),
                  row=row, col=col)
    fig.add_trace(go.Scatter(x=timesteps, y=mean['relative_diff'] - std['relative_diff'], name='mean - std', fill='tonexty', mode='lines', line_color='blue', showlegend=show_legend),
                  row=row, col=col)
...
fig.update_layout(title=f'Kp = {"{:.4E}".format(kp)}; Ki = {"{:.4E}".format(ki)}', height=3000, template='plotly_white')
fig.show()
fig.write_image(f'exports/shock_metrics-kp_{"{:.4E}".format(kp)}-ki_{"{:.4E}".format(ki)}.png', width='1800')
fig.write_html(f'exports/shock_metrics-kp_{"{:.4E}".format(kp)}-ki_{"{:.4E}".format(ki)}.html')
